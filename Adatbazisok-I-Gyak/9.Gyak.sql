...
SELECT FIZETES, KATEGORIA FROM DOLGOZO NATURAL JOIN FIZ_KATEGORIA WHERE FIZETES BETWEEN ALSO AND FELSO;
CREATE OR REPLACE FUNCTION kat_atlag(n integer) RETURN number IS
BEGIN
    DECLARE
        CURSOR CURS1 (OA NUMBER) IS SELECT FIZETES, KATEGORIA FROM DOLGOZO NATURAL JOIN FIZ_KATEGORIA WHERE FIZETES BETWEEN ALSO AND FELSO;
        REC CURS1%ROWTYPE;
        RET VARCHAR2(200);
        SUMMARY NUMBER;
        CNTR NUMBER;
    BEGIN
        ...
        LOOP
            ...
            IF(rec.KATEGORIA = n) then
                SUMMARY := SUMMARY + REC.FIZETES;
                CNTR := CNTR +1;
            end if;
        END LOOP;
        CLOSE CURS1;
